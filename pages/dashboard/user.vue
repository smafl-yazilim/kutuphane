...
async function updateMailAddress() {
  if (mailData.value.currentEmail === mailData.value.newEmail)
    return $toast.error("Yeni e-posta adresi mevcut e-posta adresiyle aynı.");

  updating.value = true;

  await authStore
    .updateEmail(mailData.value.newEmail, mailData.value.password)
    .then(() => {
      mailData.value = {
    ...
    <DashGroup>
      ...
      <DashGroupBody>
        <DashGroupInputCol
          ...
          name="current-mail"
          v-model="mailData.currentEmail"
          disabled="true"
        />
        <DashGroupInputCol
          label="Yeni E-Posta Adresi"
          name="new-mail"
          v-model="mailData.newEmail"
        />
        <DashGroupInputCol
          label="Şifre"
          type="password"
        ...
        />
        <Alert
          v-if="!mailData.verified"
          label="Doğrulanmamış E-Posta"
          theme="DANGER"
          class="my-2"
        >
          <span class="inline">
            E-Posta adresinizi doğrulamak size gönderilen
            <span class="whitespace-nowrap">e-postadaki</span> bağlantıya
            tıklayın. Spam/gereksiz kutunuzu kontrol etmeyi unutmayın.
          </span>
          <button
            class="bg-none hover:underline text-left"
